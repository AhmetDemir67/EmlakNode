              ...
              <div className="flex gap-2">
                {['Satılık', 'Kiralık'].map(t => (
                  <button key={t} type="button" onClick={() => setForm(f => ({ ...f, tip: t }))}
                    className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border-2 transition-all ${form.tip === t ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500 hover:border-blue-400'}`}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            <div className="p-6 space-y-5">
              ...
              {sonuc.ozet && (
                <div className="border border-blue-100 bg-blue-50 rounded-xl p-4">
                  <p className="text-xs font-semibold text-blue-600 mb-1">Piyasa Analizi</p>
                  <p className="text-sm text-blue-800">{sonuc.ozet}</p>
                </div>
              )}

              {sonuc.tavsiye && (
                <div className="border border-blue-100 bg-blue-50 rounded-xl p-4">
                  <p className="text-xs font-semibold text-blue-600 mb-1">Uzman Tavsiyesi</p>
                  <p className="text-sm text-blue-800">{sonuc.tavsiye}</p>
                </div>
              )}
              ...
              {sonuc.referansUzakligi && (
                <div className="border border-amber-200 bg-amber-50 rounded-xl p-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-900">{sonuc.referansUzakligi}</p>
                  </div>
                </div>
              )}
